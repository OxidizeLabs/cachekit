...
use crate::abstract_models::reference::fifo::NaiveFifoModel;
...
proptest! {
    ...
    #[test]
    fn prop_fifo_naive_matches_current_model(
        capacity in standard_capacity(),
        ops in standard_op_list(),
    ) {
        let mut naive = NaiveFifoModel::new(capacity);
        let mut current = FifoModel::new(capacity);
...
fn smoke_fifo_naive_agreement() {
    ...
    let mut naive = NaiveFifoModel::new(3);
    let mut current = FifoModel::new(3);
    assert_models_agree(&mut naive, &mut current, &ops);
}
